...
lemma ennreal_edist_of_edist_edist_le_edist
    (a b c d : ℝ≥0∞) (h : c ≠ ∞ ∨ d ≠ ∞) (h' : a ≠ ∞ ∨ b ≠ ∞):
...
lemma prod_eq_top_iff {α : Type*} (s : Finset α) (f : α → ENNReal):
    ∏ a ∈ s, f a = ⊤ ↔ (∀ a ∈ s, f a ≠ 0) ∧ (∃ a ∈ s, f a = ⊤) := by
  constructor
  · intro h
    contrapose h
...
def unit {α β : Type*} [AddMonoid β] (a : α): β → α → β :=
...
@[fun_prop]
theorem unit_measurable' {α β : Type*} [AddMonoid β] [MeasurableSpace β] (a c : α):
    Measurable (fun x : β ↦ (unit a x c)) := by
  unfold unit
  by_cases h : a = c
  · simp [h]
    exact fun ⦃t⦄ a ↦ a
...
@[fun_prop]
theorem unit_measurable {α β : Type*} [AddMonoid β] [MeasurableSpace α] [MeasurableSpace β]
    (a : α) (b : β) (ha : MeasurableSet {a}): Measurable (unit a b) := by
  unfold unit
...
lemma iterate'_up_to_equal_up
    {m n : (Fin (Nat.card ι + 1))} (mn : m = n) (S : ι → X → X) (k : (Fin (Nat.card ι)) → ℕ):
    iterate'_up_to m S k = iterate'_up_to n S k := by rw [mn]
...
lemma iterate'_up_to_add_one
  {m : ℕ} (hm : m + 1 < Nat.card ι + 1) (S : ι → X → X) (k : (Fin (Nat.card ι) → ℕ)):
  iterate'_up_to ⟨m + 1, hm⟩ S k
  = (S ((Finite.equivFin ι).symm ⟨m, lt_add_one_down hm⟩))^[k ⟨m, Nat.succ_lt_succ_iff.mp hm⟩]
  ∘ (iterate'_up_to ⟨m, Nat.lt_of_succ_lt hm⟩ S k) := by
    ext x
...
lemma iterate'_up_to_is_quick
    (m : ℕ) (hm : m < (Nat.card ι) + 1) (S : ι → X → X) (k : Fin (Nat.card ι) → ℕ):
    iterate'_up_to_quick m S k = iterate'_up_to ⟨m,hm⟩ S k := by
  unfold iterate'_up_to_quick
...
lemma iterate'_up_to_neg_one {m : ℕ} (hm : m < Nat.card ι + 1) (hm' : m ≠ 0) (S : ι → X → X) (k : (Fin (Nat.card ι) → ℕ)):
  iterate'_up_to ⟨m, hm⟩ S k = (S ((Finite.equivFin ι).symm ⟨m-1, sub_one_lt hm hm'⟩))^[k ⟨m-1, sub_one_lt hm hm'⟩] ∘ (iterate'_up_to ⟨m-1, tsub_lt_of_lt hm⟩ S k) := by
    ...
    have : iterate'_up_to_quick m S k = iterate'_up_to_quick (↑(NatCast.natCast n+ 1)) S k := by
      rw [hn]
      simp
    rw [this, iterate'_up_to_is_quick, iterate'_up_to_add_one]
    swap
    · unfold n
      simp
...
theorem iterate'_up_to_end_zero
    (m : Fin (Nat.card ι + 1)) (hm : m ≠ 0) (S : ι → X → X) (k : Fin (Nat.card ι) → ℕ) (hk : k ⟨m - 1, fin_add_neg_one hm⟩ = 0):
    iterate'_up_to m S k = iterate'_up_to (m-1) S k := by
...
theorem iterate'_up_to_same
    (m : Fin (Nat.card ι + 1)) (S : ι → X → X) {k k' : Fin (Nat.card ι) → ℕ}
    (h : ∀ n : Fin (Nat.card ι), (↑n : ℕ) < m → k n = k' n):
...
theorem iterate'_up_to_add (m : Fin (Nat.card ι + 1)) {S : ι → X → X} (hS : pairwise_commuting S) (k k' : Fin (Nat.card ι) → ℕ):
  iterate'_up_to m S (k + k') = iterate'_up_to m S k ∘ iterate'_up_to m S k' := by
    ...
    all_goals try exact Ne.symm (ne_of_beq_false rfl)
...
theorem iterate'_up_to_unit
    (m : Fin (Nat.card ι + 1)) (S : ι → X → X) (i : Fin (Nat.card ι)) (r : ℕ):
    iterate'_up_to m S (unit i r) = if (↑i : ℕ) < m then (S ((Finite.equivFin ι).symm i))^[r] else id := by
  obtain ⟨m,hm⟩ := m
  induction' m with m hm'
  · simp
...
theorem iterate'_unit (S : ι → X → X) (i : Fin (Nat.card ι)) (r : ℕ):
    iterate' S (unit i r) = (S ((Finite.equivFin ι).symm i))^[r] := by
  unfold iterate'
...
@[simp]
theorem iterate_zero (S : ι → X → X): iterate S 0 = id := by
  unfold iterate
  rw [← iterate'_zero S]
  rfl
...
theorem iterate_add' {S : ι → X → X} (hS : pairwise_commuting S) (k k' : ι → ℕ):
    iterate S (k+k') = iterate S k ∘ iterate S k' := by
  unfold iterate
  rw [← iterate'_add]
...
theorem iterate'_up_to_measurepreserving
    (m : Fin (Nat.card ι + 1)) {S : ι → Z → Z} (hS : MMeasurePreserving μ S) (k : Fin (Nat.card ι) → ℕ):
...
@[fun_prop]
theorem iterate'_up_to_measurable
    (m : Fin (Nat.card ι + 1)) {S : ι → Z → Z} (hS : ∀ i : ι, Measurable (S i)) (k : Fin (Nat.card ι) → ℕ):
    Measurable (iterate'_up_to m S k) := by
  obtain ⟨m,hm'⟩ := m
  induction' m with m hm
  · simp
    fun_prop
...
@[simp]
theorem iterate'_measurepreserving {S : ι → Z → Z} (hS : MMeasurePreserving μ S) (k : Fin (Nat.card ι) → ℕ):
  MeasurePreserving (iterate' S k) μ μ := iterate'_up_to_measurepreserving μ ⟨Nat.card ι, lt_add_one (Nat.card ι)⟩ hS k

@[fun_prop]
theorem iterate'_measurable {S : ι → Z → Z} (hS : ∀ i : ι, Measurable (S i)) (k : Fin (Nat.card ι) → ℕ): Measurable (iterate' S k) := by
  unfold iterate'
  fun_prop

@[simp]
theorem iterate_measurepreserving {S : ι → Z → Z} (hS : MMeasurePreserving μ S) (k : ι → ℕ): MeasurePreserving (iterate S k) μ μ := by
  unfold iterate
...
@[fun_prop]
theorem iterate_measurable {S : ι → Z → Z} (hS : ∀ i : ι, Measurable (S i)) (k : ι → ℕ):
    Measurable (iterate S k) := by
  unfold iterate
  fun_prop
...
@[fun_prop]
theorem ergodic_avg_measurable
    (n : ℕ) (S : ι → Z → Z) (f : ι → Z → ℝ) (hf : ∀ (i : ι), Measurable (f i)) (hS : ∀ (j : ι), Measurable (S j)):
...
def multilinear_avg (φ : ℝ → ℝ) (F : ι → (ι → ℝ) → ℝ) (t : NNReal): (ι → ℝ) → ℝ :=
...
def push_forward (N : ℕ) (S : ι → X → X) (F : X → ℝ) (x : X): (ι → ℤ) → ℝ :=
  fun k ↦ if ∀ i : ι, k i < 2*N ∧ 0 ≤ k i then F (iterate S (fun i ↦ (k i).toNat) x) else 0

@[fun_prop]
theorem push_forward_measurable [MeasurableSpace X] (N : ℕ) (k : ι → ℤ)
    (S : ι → X → X) (F : X → ℝ) (hS : ∀ (i : ι), Measurable (S i)) (hF : Measurable F):
    Measurable (fun x ↦ push_forward N S F x k) := by
      unfold push_forward
      by_cases h : ∀ (i : ι), k i < 2 * N ∧ 0 ≤ k i
      all_goals simp [h]
      fun_prop


def push_forward_many (N : ℕ) (S : ι → X → X) (F : ι → (X → ℝ)) (x : X): ι → (ι → ℤ) → ℝ :=
  ...
  multilinear_avg_spec F t x = (1 /t) * ∫ (s : ℝ) in Ico (∑ (i : ι), x i) ((∑ (i : ι), x i) + t), ∏(j : ι), (F j) (x + Calderon.unit j (s - ∑ (j : ι), (x j))) := by
    ...
    calc
      ∫ (s : ℝ), (Ico 0 1).indicator 1 (1 / ↑t * s) * ∏ j, F j (x + unit j s) =  ∫ (s : ℝ), (Ico 0 1).indicator 1 (1 / ↑t * s) * ∏ j, F j (x + unit j s) := by
        congr
    _ = ∫ (s : ℝ) in Ico 0 ↑t, ∏ j, F j (x + unit j s) := by
...
lemma set_fun_cover (f : γ → α → β) {s : γ → Set α} {a : α} (hs : ∀ a : α, ∃ c : γ, a ∈ s c):
    ∃ i : γ, set_fun f s a = f i a := by
  use (hs a).choose
  unfold set_fun
  simp [hs a]
...
@[fun_prop]
theorem round_down_meausurable (i : ι): Measurable (round_down i) := by
...
def interlude_sum (k : ι → ℤ) (i : ι) (f : (ι → ℤ) → ℝ): ℝ → ℝ :=
  set_fun (fun a _ ↦ f (k + unit i (a + ∑ r : ι, k r))) (fun (a : ℤ) ↦ Ico a (a+ 1))
...
@[fun_prop] lemma approx_sum_measurable (i : ι) {F : (ι → ℤ) → ℝ}: Measurable (approx_sum i F) := by
  rw [approx_sum_round_down]
  fun_prop
...
theorem tsum_prod_set
    {α β : Type*} (s : Finset β) (ne : s ≠ ∅) {f : β → α → ENNReal}
    (h : ∀ a a' : α, ∀ i i' : β, a ≠ a' → i ≠ i' → f i a * f i' a' = 0):
...
theorem tsum_prod
    [Nonempty ι] {α : Type*} {f : ι → α → ENNReal}
    (h : ∀ a a' : α, ∀ i i' : ι, a ≠ a' → i ≠ i' → f i a * f i' a' = 0):
...
theorem tsum_prod_set_real
    {α β : Type*} (s : Finset β) (ne : s ≠ ∅) {f : β → α → ℝ}
    (h : ∀ a a' : α, ∀ i i' : β, a ≠ a' → i ≠ i' → f i a * f i' a' = 0):
    ∏ i ∈ s, ∑' a : α, f i a = ∑' a : α, ∏ i ∈ s,  f i a := by
  revert s
  apply Finset.induction
...
theorem tsum_prod_real
    [Nonempty ι] {α : Type*} {f : ι → α → ℝ}
    (h : ∀ a a' : α, ∀ i i' : ι, a ≠ a' → i ≠ i' → f i a * f i' a' = 0):
    ∏ i : ι, ∑' a : α, f i a = ∑' a : α, ∏ i : ι,  f i a := by
...
theorem fact5_4 [Nonempty ι] (k : ι → ℤ) (α : ι → ℝ) (hα : ∀ i : ι, 0 ≤ α i ∧ α i < 1)
    (F : ι → (ι → ℤ) → ℝ) {t : NNReal} (ht : 0 < t):
    (multilinear_avg_spec (approx_sum_mul F) t (fun j ↦ k j + α j)) =
    (1 / t * ∑' i : ℤ, (volume (Ico (i : ℝ) (i + 1) ∩ Ico (∑ (j : ι), (k j + α j)) (t + ∑ (j : ι), (k j + α j)))).toReal *
    ∏(j_1 : ι), (F j_1) (fun o ↦ ((k o : ℤ) + unit j_1 (i - (∑ (r : ι), (k r))) o))):= by
  rw [fact5_1]
  swap
  · contrapose ht
    simp_all
...
theorem ell_p_coe {α : Type*} {f : α → ℝ} (hf : 0 ≤ f):
  ...
  · have ns : ¬ Summable fun a ↦ (f a).toNNReal := by
    ...
    exact hf a
  suffices : ∑' (a : α), ‖f a‖ₑ = ∑' (a : α), (f a).toNNReal
  · rw [this]
...
lemma setup_discrete_ver'_norm_two {g : ℕ → NNReal} {q : ι → ℝ} (hg : good g q) (hq : (∀ (i : ι), 0 < q i) ∧ ∑ (i : ι), 1 / (q i) = 1 / 2) (m : ℕ)
  ...
  unfold u_diff
  apply fact5_2 (F := fun i ↦ approx_sum i (F i)) (h := hg) (m := m) (idx := fun i ↦ (↑(idx i) : NNReal))
  · intro a b ab
...
theorem multilinear_avg_approx_edist_udiff
    {q : ι → ℝ} (F : ι → (ι → ℤ) → ℝ) (m : ℕ)
...
theorem discrete_ver'_norm_one_individual_preadd {g : ℕ → NNReal} {q : ι → ℝ} (hg : good g q) (hq : (∀ (i : ι), 0 < q i) ∧ ∑ (i : ι), 1 / (q i) = 1 / 2) (m : ℕ)
  ...
  have : ENNReal.ofReal (16*(Fintype.card ι) ^ (3 / (2 : ℝ)) / ↑(idx ⟨i, get_rid i⟩))
      = (16*(Fintype.card ι) ^ (3 / (2 : ℝ)) / (↑(idx ⟨i, get_rid i⟩) : NNReal)) := by
    rw [ENNReal.ofReal_div_of_pos]
    swap
...
theorem discrete_ver'_norm_one_individual_add_sq
    {g : ℕ → NNReal} {q : ι → ℝ} (hg : good g q)
    (hq : (∀ (i : ι), 0 < q i) ∧ ∑ (i : ι), 1 / (q i) = 1 / 2) (m : ℕ)
    (idx : Fin (m + 1) → ℕ) (mon : Monotone idx) (hidx : Injective idx) (hidx' : 0 < idx 0)
    (F : ι → (ι → ℤ) → ℝ) (hF : ∀ (j : ι), Memℓp (F j) (q j).toNNReal)
    (hF' : ∀ (j : ι), ∑' a : ι → ℤ, |(F j a)| ^ (q j) = 1) (i : Fin m):
...
theorem fact5_5 (hS : pairwise_commuting S) (k : ι → ℤ) (hk : 0 ≤ k) (hN : ∀ (i : ι), (k i + n) < 2 * N) (x : X):
  ergodic_avg n S f (iterate S (fun i ↦ (k i).toNat) x) = discrete_avg' n ((push_forward_many N S f) x) k := by
    ...
    suffices : ∀ (i_1 : ι), (k + unit i (↑u : ℤ)) i_1 < 2 * N ∧ 0 ≤ (k + unit i (↑u : ℤ)) i_1
    · simp only [this]
      simp
      congr 1
      have : (fun i_1 ↦ (k i_1 + unit i (↑u) i_1).toNat) = (fun i_1 ↦ (k i_1).toNat) + fun i_1 ↦ (unit i (↑u :ℤ) i_1).toNat := by
        ext r
        simp
        refine Int.toNat_add (hk r) ?_
        unfold unit
        by_cases ir : i = r <;> simp [ir]
      rw [this, add_comm, iterate_add' (hS := hS)]
      simp
      congr
      unfold unit
      ext j
      by_cases ij : i = j <;> simp [ij]
    simp
    intro j
    constructor
    swap
    · apply add_nonneg (hk j)
      unfold unit
      by_cases ij : i = j <;> simp [ij]
    calc
      (k j + unit i (↑u : ℤ) j)
...
def icoset (ι : Type*) (a b : ℤ): Set (ι → ℤ) :=
  {k | ∀ i, a ≤ k i ∧ k i < b}

theorem icoset_encard
    {a b : ℤ} (ι : Type*) [Fintype ι]:
    encard (α := ι → ℤ) (icoset ι a b) = (b - a).toNat^(Fintype.card ι) := by
...
def bound_set : Set (ι → ℤ) := icoset ι 0 (↑n : ℤ)

omit [Nonempty ι]
theorem bound_set_encard : encard (α := ι → ℤ) (bound_set N) = N^(Fintype.card ι) := by
  unfold bound_set
  rw [icoset_encard]
  simp
...
theorem push_forward_bound_set (F : X → ℝ) (x : X): support (push_forward N S F x) ⊆ (bound_set (2*N)) := by
  ...
  simp
  by_cases N0: N ≤ 0
  · unfold push_forward bound_set icoset at *
    simp_all
  unfold bound_set icoset at kh
  unfold push_forward
  simp_all
  have : 2 * (↑N : ℤ) = 2*N := by simp
  rw [this]
  intro h
  contrapose kh
  simp
...
theorem push_forward_not_in_bound_set {k : ι → ℤ} (F : X → ℝ) (x : X) (hk : k ∉ @bound_set ι (2 * N)): push_forward N S F x k = 0 := by
  contrapose hk
  simp
...
instance bound_set_finite : Finite (bound_set (ι := ι) N) := by
...
theorem fact5_6 (a b : ℕ) [MeasurableSpace X] (μ : Measure X) (hS : pairwise_commuting S) (hS' : MMeasurePreserving μ S)
  ...
  have : ∫⁻ (x : X), ‖(ergodic_avg a S f x - ergodic_avg b S f x)‖ₑ ^ 2 ∂μ = 1 / N^p * ∑ x ∈ (@bound_set ι N).toFinset, ∫⁻ (x : X), ‖(ergodic_avg a S f x - ergodic_avg b S f x)‖ₑ ^ 2 ∂μ := by
    ...
    simp
    have : (Fintype.card (@bound_set ι N)) = encard (α := ι → ℤ) (bound_set N) := by simp
    have : (Fintype.card (@bound_set ι N)) = (encard (α := ι → ℤ) (bound_set N)).toNat := by
      rw [← this]
      rfl
  ...
  calc
        ...
        ∑ k ∈ (bound_set N).toFinset, ∫⁻ (x : X), ‖ergodic_avg a S f (iterate S (fun i ↦ (k i).toNat) x) - ergodic_avg b S f (iterate S  (fun i ↦ (k i).toNat) x)‖ₑ ^ 2 ∂μ := by
          apply Finset.sum_congr rfl
          intro k _
          rw [MeasureTheory.MeasurePreserving.lintegral_comp (μ := μ) (ν := μ) (f := fun x ↦ ‖ergodic_avg a S f x - ergodic_avg b S f x‖ₑ ^ 2) (g := fun x ↦ iterate S (fun i ↦ (k i).toNat) x)]
    ...
    _ = ∫⁻ (x : X), ∑ k ∈ (bound_set N).toFinset, ‖ergodic_avg a S f (iterate S (fun i ↦ (k i).toNat) x) - ergodic_avg b S f (iterate S (fun i ↦ (k i).toNat) x)‖ₑ ^ 2 ∂μ := by
    ...
    _ = ∫⁻ (x : X), ∑ k ∈ (bound_set N).toFinset, ‖discrete_avg' a ((push_forward_many N S f) x) k - discrete_avg' b ((push_forward_many N S f) x) k‖ₑ ^ 2 ∂μ := by
          ...
          apply Finset.sum_congr rfl
          intro x xh
          have xh' : x ∈ bound_set N := mem_toFinset.mp xh
          unfold bound_set icoset at xh'
          simp at xh'
          rw [← fact5_5 (hS := hS), ← fact5_5 (hS := hS)] <;> intro i <;> specialize xh' i <;> try tauto
          · calc
              x i + ↑b
                < (↑N : ℤ) + ↑b := by linarith
              _ ≤ (↑N : ℤ) + (↑N : ℤ) := by
    ...
    _ = ∫⁻ (x : X), ‖∑' k : ι → ℤ, (discrete_avg' a (push_forward_many N S f x) k - discrete_avg' b (push_forward_many N S f x) k) ^ 2‖ₑ ∂μ := by
          ...
          · simp
          suffices : ¬∀ (i_1 : ι), u i_1 + unit i (↑k : ℤ) i_1 < 2 * N ∧ 0 ≤ u i_1 + unit i (↑k) i_1
          · simp [this]
          simp
          obtain ⟨y,yh⟩ := uh
          use y
          ...
          simp_all
          constructor
          · refine Int.add_nonnneg_iff_neg_le.mp ?_
            apply le_of_lt
            apply Int.lt_of_le_of_lt yh
            simp
            unfold unit
            by_cases iy : i = y <;> simp [iy]
            · linarith
            exact Nat.zero_lt_of_ne_zero hN
          apply Int.lt_of_le_of_lt ?_ h
          unfold unit
          by_cases iy : i = y <;> simp [iy]
...
theorem fact5_7 [MeasurableSpace X] (μ : Measure X) (q : ℝ) (hS : MMeasurePreserving μ S) (f : X → ℝ) (hf : Measurable f) (hq : 0 < q):--vermutlich kann man hq vermeiden, aber sonst mach 0^q probleme mit coercions, ein calc step ist dann au jeden fall einfach falsch wenn q=0 (der erste if i am not mistaken)
  ∫⁻ (x : X), ‖∑' (k : ι → ℤ), |(push_forward N S f x k)|^q‖ₑ ∂μ = 2^(Fintype.card ι) * N^(Fintype.card ι) * ∫⁻ x : X, ‖f x‖ₑ^q ∂μ := by
    by_cases hι: IsEmpty ι
    ...
    simp at hι
    calc
      ∫⁻ (x : X), ‖∑' (k : ι → ℤ), |push_forward N S f x k| ^ q‖ₑ ∂μ
        = ∫⁻ (x : X), ∑' (k : ι → ℤ), ‖push_forward N S f x k‖ₑ^ q ∂μ := by
          apply lintegral_congr
          intro x
          rw [enorm_eq_ofReal_abs]
          have : |∑' (k : ι → ℤ), |push_forward N S f x k| ^ q| = ∑' (k : ι → ℤ), |push_forward N S f x k| ^ q := by
            simp
            apply tsum_nonneg
            intro k
            apply rpow_nonneg
          ...
          intro k kh
          contrapose kh
          unfold bound_set icoset push_forward at *
          simp_all [-not_forall]
          have : ¬∀ (i : ι), k i < 2 * ↑N ∧ 0 ≤ k i := by
            contrapose kh
            simp_all
          simp [this]
          rw [Real.zero_rpow]
      ...
      _ = ∑ k ∈ bound_set (ι := ι) (2*N), ∫⁻ (x : X), ‖f x‖ₑ^ q ∂μ := by
          apply Finset.sum_congr rfl
          intro k kh
          have : ∫⁻ (x : X), ‖push_forward N S f x k‖ₑ ^ q ∂μ = ∫⁻ (x : X), ‖f (iterate S (fun i ↦ (k i).toNat) x)‖ₑ ^ q ∂μ := by
            apply lintegral_congr
            intro x
            unfold bound_set icoset at kh
            unfold push_forward
            simp_all
          rw [this]
          apply MeasureTheory.MeasurePreserving.lintegral_comp (g := fun x ↦ iterate S (fun i ↦ (k i).toNat) x) (f := fun x ↦ ‖f x‖ₑ^q)
          · have : (fun x ↦ iterate S (fun i ↦ (k i).toNat) x) = iterate S (fun i ↦ (k i).toNat) := rfl
            rw [this]
            apply iterate_measurepreserving
            exact hS
          fun_prop [hf]
      _ = 2^(Fintype.card ι) * N^(Fintype.card ι) * ∫⁻ x : X, ‖f x‖ₑ^q ∂μ := by
          simp
          --have : Fintype.card (bound_set (2 * N)) = encard (α := ι → ℤ) (bound_set (ι := ι) (2*N)) := by
          have : (↑(Fintype.card (bound_set (ι := ι) (2 * N))) : ENNReal) = encard (α := ι → ℤ) (bound_set (ι := ι) (2*N)) := by
            suffices : Fintype.card (bound_set (ι := ι) (2 * N)) = encard (α := ι → ℤ) (bound_set (ι := ι) (2*N))
            · rw [← this]
              dsimp
            simp
          rw [this, bound_set_encard]
...
theorem goal' [h : Nonempty ι] {f : ι → X → ℝ} {g : ℕ → NNReal} {q : ι → ℝ} (hg : good g q) [MeasurableSpace X] (μ : Measure X) (hq : (∀ (i : ι), 0 < q i) ∧ ∑ (i : ι), 1 / (q i) = 1 / 2)
  (m : ℕ) (idx : Fin (m + 1) → ℕ) (mon : Monotone idx) (hidx : Injective idx) (hidx' : 0 < idx 0) (hS : MMeasurePreserving μ S) (hS' : pairwise_commuting S) (hf : ∀ (i : ι), Measurable (f i)): --(hf : ∀ (i : ι), MeasureTheory.MemLp (f i) (q i) μ) Probably hf isnt needed, maybe (aestrongly)measurable or smth??? Something along those lines definetely (else integrals become zero on RHS)
          ...
          (discrete_avg' (idx b.succ) (push_forward_many N S f x) k -
            ...
            2‖ₑ) := by
              ...
              · unfold Q
                ...
                apply le_trans kh.1
                unfold unit
                by_cases jx : j = x <;> simp_all
                linarith
              unfold P
              apply Int.lt_of_le_of_lt _ kh.2
              unfold unit
              by_cases jx : j = x <;> simp [jx]
            rw [this]
            apply Measurable.comp
            · exact measurable_enorm
            refine Finset.measurable_sum (icoset ι Q P).toFinset ?_
            intro k _
            ...
            repeat
              ...
              exact (hS i).measurable
              exact hf j
        _ ≤ 1 / N^(Fintype.card ι) * ∫⁻ (x : X), C * ↑(g m) * ∑ (j : ι), ‖(∑' a : ι → ℤ, |((push_forward_many N S f) x) j a| ^ (q j))‖ₑ ∂μ := by
            gcongr
            unfold C
            rename_i x
    ...
    ∑ j, ‖∑' (a : ι → ℤ), |push_forward_many N S f x j a| ^ q j‖ₑ = ‖(↑(256 * (↑(Fintype.card ι) : ℝ) ^ 3 * π ^ 2 / 3).toNNReal + 2 * ↑(good_const hg)) * ↑(g m) *
    ∑ j, ∑' (a : ι → ℤ), |push_forward_many N S f x j a| ^ q j‖ₑ := by
              rw [enorm_mul]
              congr
              · rw [enorm_mul]
                congr
                ...
                simp
              rw [enorm_eq_ofReal_abs]
              have : |∑ j, ∑' (a : ι → ℤ), |push_forward_many N S f x j a| ^ q j| = ∑ j, ∑' (a : ι → ℤ), |push_forward_many N S f x j a| ^ q j := by
                simp
                apply Finset.sum_nonneg
                intros
                apply tsum_nonneg
              ...
              · apply Finset.sum_congr rfl
                intro y _
                rw [enorm_eq_ofReal_abs]
                suffices : |∑' (a : ι → ℤ), |push_forward_many N S f x y a| ^ q y| = ∑' (a : ι → ℤ), |push_forward_many N S f x y a| ^ q y
                · rw [this]
                simp
                apply tsum_nonneg
                intros
            ...
            · simp
              ...
              simp_all
            exact hk
        _ = 1 / N^(Fintype.card ι) * C * ↑(g m) * ∫⁻ (x : X), ∑ (j : ι), ‖(∑' a : ι → ℤ, |((push_forward_many N S f) x) j a| ^ (q j))‖ₑ ∂μ := by
            rw [MeasureTheory.lintegral_const_mul']
            · repeat rw [mul_assoc]
            exact Ne.symm (not_eq_of_beq_eq_false rfl)
        _ = 1 / N^(Fintype.card ι) * C * ↑(g m) * ∑ (j : ι), ∫⁻ (x : X), ‖(∑' a : ι → ℤ, |((push_forward_many N S f) x) j a| ^ (q j))‖ₑ ∂μ := by
            congr
            rw [MeasureTheory.lintegral_finset_sum]
            intro b _
            refine Measurable.enorm ?_
            have : (fun x ↦ ∑' (a : ι → ℤ), |push_forward_many N S f x b a| ^ q b) = (fun x ↦ ∑ a ∈ bound_set (2*N), |push_forward_many N S f x b a| ^ q b) := by
              ext x
              apply tsum_eq_sum
              intro k kh
              unfold push_forward_many
        ...
        _ = 1 / N^(Fintype.card ι) * C * ↑(g m) * ∑ (j : ι), (2^(Fintype.card ι) * N^(Fintype.card ι) * ∫⁻ x_1 : X, ‖f j x_1‖ₑ^(q j) ∂μ) := by
            suffices : ∑ j, ∫⁻ (x : X), ‖∑' (a : ι → ℤ), |push_forward_many N S f x j a| ^ q j‖ₑ ∂μ = ∑ (j : ι), (2^(Fintype.card ι) * N^(Fintype.card ι) * ∫⁻ x_1 : X, ‖f j x_1‖ₑ^(q j) ∂μ)
            · rw [this]
            apply Finset.sum_congr rfl
            intro i _
            rw [← fact5_7 (S := S)]
     ...
     g m * ∏(i : ι), (∫⁻ (x : X), ‖f i x‖ₑ^(q i) ∂μ)^(2/ (q i)) := by
      ...
      calc
        ...
        _ = ∑ i : Fin m, ‖(∏ i : ι, (r i).toReal)‖ₑ^2 * ∫⁻ (x : X), ‖(ergodic_avg (idx i.succ) S (fun j y ↦ ((f j y) / (r j).toReal)) x - ergodic_avg (idx ⟨↑i, get_rid i⟩) S (fun j y ↦ ((f j y) / (r j).toReal)) x)‖ₑ ^ 2 ∂μ := by
          ...
          intro i
          exact (hS i).measurable
        _ = ‖∏ i : ι, (r i).toReal‖ₑ^2 *  ∑ i : Fin m, ∫⁻ (x : X),
            ‖(ergodic_avg (idx i.succ) S (fun j y ↦ ((f j y) / (r j).toReal)) x -
            ergodic_avg (idx ⟨↑i, get_rid i⟩) S (fun j y ↦ ((f j y) / (r j).toReal)) x)‖ₑ ^ 2 ∂μ := by rw [Finset.mul_sum]
